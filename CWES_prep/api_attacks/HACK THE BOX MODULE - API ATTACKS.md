...
After eliminating every out-of-scope endpoints, we can now start hunting for the vulnerable one(s), and soon enough, we found the endpoint `GET /api/v1/customers/billing-addresses` being vulnerable to BFLA.![image](photos/api_4.png)
...
- APIs can also suffer from security misconfigurations if they do not use proper HTTP Security Response Headers. In that case, it can be exposed to security risks, most notably, Cross-Site Request Forgery (CSRF).

### Target:
- Inlanefreight E-Commerce Marketplace API endpoints : *83.136.249.164:41411*

- Authenticate to 83.136.249.164:41411 with user "htbpentester13@hackthebox.com" and password "HTBPentester13"

### - Challenge 1
- Exploit another Security Misconfiguration and provide the total count of records within the target table.

--
After login in, we start by checking our user's roles:
...
*Suppliers_GetTotalCountBySupplierNameSubstring* allows user to use `GET /api/v1/suppliers/{Name}/count` API.

By providing the string `test'` as the input, we can see that the server response an error: ![image](photos/api_6.png) 
...
``` 
The error shows a sign of potential SQL injection vulnerablity. To prove that the endpoint is vulnerable, we test it with `test' OR 1=1 -- -` to get the total suppliers count the current table.
...
```
We got `151` as the record count in this table as the flag and proved the vulnerability.

### - Challenge 2
- Submit the header and its value that expose another Security Misconfiguration in the API.

--
Here we tried to edit a new header into the request to this api then send it again, we got a new header in the response body:

![image](photos/api_7.png)
- `Access-Control-Allow-Origin: *`:  *The requesting code from any origin is allowed to access the resource.* [[*]](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Access-Control-Allow-Origin)
- The new header shows that this API not only vulnerable to SQL injection but also CSRF.

---

## Improper Inventory Management
- Improper inventory management of APIs, including *inadequate versioning*, can introduce security misconfigurations and increase the attack surface.
...
Exploit the `GET /api/v0/supplier-companies/deleted` endpoint to find the infomation of the supplier company with the ID `c250cb38-96e3-4ccf-9df2-0a03146a2d0b`:
- Response body:
```
...
  {
...
...
```
We got their email as our flag.
...
(I need someone who understand this better to explain this for me)

All I did was taking the password hash from `GET /api/v0/suppliers/deleted` and that's it.
